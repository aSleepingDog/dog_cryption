...
void dog_cryption::Cryptor::decryptp(std::istream& crypt, std::ostream& plain, bool with_config, bool with_iv, dog_data::Data iv, bool with_check,
	std::mutex* mutex_, std::condition_variable* cond_, std::atomic<double>* progress, std::atomic<bool>* running_, std::atomic<bool>* paused_, std::atomic<bool>* stop_)
{
	std::unique_ptr<dog_cryption::CryptionConfig> ori_config;
	if (with_config)
...
//Twofish
uint8_t dog_cryption::Twofish::mult14D(uint8_t a, uint8_t b)
{
	if (a == 0x00 || b == 0x00)
	{
		return 0x00;
...
uint8_t dog_cryption::Twofish::mult169(uint8_t a, uint8_t b)
{
	if (a == 0x00 || b == 0x00)
	{
		return 0x00;
	}
	return GF169[((GFi169[a - 1] + GFi169[b - 1])) % 255];
}
uint32_t dog_cryption::Twofish::multMDS(uint32_t n)
{
	uint8_t n_[4] = {
			dog_number::integer::pick_byte(n,4),
			dog_number::integer::pick_byte(n,3),
			dog_number::integer::pick_byte(n,2),
			dog_number::integer::pick_byte(n,1), };
	uint8_t s[4] = { 0,0,0,0 };
	for (uint8_t row = 0; row < 4; row++)
	{
		for (uint8_t column = 0; column < 4; column++)
		{
			s[row] ^= mult169(MDS[row * 4 + column], n_[column]);
		}
	}
	return s[0] | (s[1] << 8) | (s[2] << 16) | (s[3] << 24);
}
uint32_t dog_cryption::Twofish::function_g(uint32_t n)
{
	return 0;
}
uint32_t dog_cryption::Twofish::function_h(uint32_t x, std::vector<uint32_t> l)
{
	uint8_t y[4] = { 
		dog_number::integer::pick_byte(x,4),
		dog_number::integer::pick_byte(x,3),
		dog_number::integer::pick_byte(x,2),
		dog_number::integer::pick_byte(x,1),
	};
	if (l.size() == 4)
	{
		y[0] = P8x8[1][y[0]] ^ dog_number::integer::pick_byte(l[3], 4);
		y[1] = P8x8[0][y[1]] ^ dog_number::integer::pick_byte(l[3], 3);
		y[2] = P8x8[0][y[2]] ^ dog_number::integer::pick_byte(l[3], 2);
		y[3] = P8x8[1][y[3]] ^ dog_number::integer::pick_byte(l[3], 1);
	}
	if (l.size() >= 3)
	{
		y[0] = P8x8[1][y[0]] ^ dog_number::integer::pick_byte(l[2], 4);
		y[1] = P8x8[1][y[1]] ^ dog_number::integer::pick_byte(l[2], 3);
		y[2] = P8x8[0][y[2]] ^ dog_number::integer::pick_byte(l[2], 2);
		y[3] = P8x8[0][y[3]] ^ dog_number::integer::pick_byte(l[2], 1);
	}
	y[0] = P8x8[1][P8x8[0][P8x8[0][y[0]] ^ dog_number::integer::pick_byte(l[1], 4)] ^ dog_number::integer::pick_byte(l[0], 4)];
	y[1] = P8x8[0][P8x8[0][P8x8[1][y[1]] ^ dog_number::integer::pick_byte(l[1], 3)] ^ dog_number::integer::pick_byte(l[0], 3)];
	y[2] = P8x8[1][P8x8[1][P8x8[0][y[2]] ^ dog_number::integer::pick_byte(l[1], 2)] ^ dog_number::integer::pick_byte(l[0], 2)];
	y[3] = P8x8[0][P8x8[1][P8x8[1][y[3]] ^ dog_number::integer::pick_byte(l[1], 1)] ^ dog_number::integer::pick_byte(l[0], 1)];
	uint32_t Y = y[0] << 24 | y[1] << 16 | y[2] << 8 | y[3];
	return multMDS(Y);
}
dog_data::Data dog_cryption::Twofish::extend_key(dog_data::Data& key, uint64_t key_size)
{
	...
	for (uint64_t i = 0; i < 20; i++)
	{
		...
		uint32_t B = dog_number::integer::CLMB(function_h((2 * i + 1) * p, Mo), 8);
		rkey[i * 2] = A + B;
		rkey[i * 2 + 1] = dog_number::integer::CLMB(A + 2 * B, 9);
	}
