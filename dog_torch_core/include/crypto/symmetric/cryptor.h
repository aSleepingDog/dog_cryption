...
namespace dog_torch { namespace crypto {namespace symmetric
{
	...
	private:
		...
		std::function<Data(Data&, uint64_t)> extend_key_;//密钥扩展方法
		...
		using block_cryption_func      = std::function<Data(Data&, uint64_t, const Data&, uint64_t)>;
	...
	namespace <加密算法>
	{
		...
		//密钥加工
		Data extend_key(Data& key, uint64_t key_size);
		//加密有返回
		Data encoding(Data& plain, uint64_t block_size, const Data& key, uint64_t key_size);
		Data decoding(Data& crypt, uint64_t block_size, const Data& key, uint64_t key_size);
		//加密无返回
