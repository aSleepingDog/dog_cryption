...
public slots:
	void receive()
	{
		...
		for (auto& hash : dog_torch::crypto::hash::list)
		{
...
public slots:
	void receive()
	{
		...
		for (auto& padding : dog_torch::crypto::symmetric::padding::list)
		{
			QJsonObject single_padding;
			single_padding["name"] = QString::fromStdString(padding.name);
			single_padding["code"] = padding.code * 1.0;
...
public slots:
	void receive()
	{
		...
		for (auto& mode : dog_torch::crypto::symmetric::mode::list)
		{
			QJsonObject single_mode;
			single_mode["name"] = QString::fromStdString(mode.name);
			single_mode["code"] = mode.code * 1.0;
			single_mode["forceIv"] = mode.force_iv;
			single_mode["forcePadding"] = mode.force_padding;
			single_mode["forceShift"] = mode.force_shift;
...
public slots:
	void receive()
	{
		...
		for (auto& algorithm : dog_torch::crypto::symmetric::Algorithm_list)
		{
...
public slots:
	void receive(const QString& jsonstr)
	{
		try
		{
			...
			dog_torch::serialize::json::Object dog_params = std_json_str;
			//std::cout << std_json_str << std::endl;
			dog_param::IOConfig input__config = dog_param::IOConfig(dog_params["input"].to_object(), true);
			dog_param::IOConfig output_config = dog_param::IOConfig(dog_params["output"].to_object(), false);
	...
	void get_data_size(const QString& jsonstr)
	{
		...
		try
		{
			...
			dog_param::IOConfig input__config = dog_param::IOConfig(dog_params["input"].to_object(), true);
			dog_torch::serialize::Data data = input__config.get_data();
			result["code"] = 0;
			result["msg"] = "success";
			QJsonValue size = data.size() < 0x20000000000000 ? QJsonValue(data.size() * 1.0) : QJsonValue("overflow");
			result["size"] = size;
			result["id"] = QString::fromStdString(dog_params["id"].to_string());
		}
		catch (std::exception& e)
		{
			result["code"] = 1;
...
public slots:
	void work(const QString& jsonstr)
	{
		...
		try
		{
			...
			std::unique_ptr<dog_torch::crypto::hash::HashConfig> hash_config = nullptr;
			for (auto& single_config : dog_torch::crypto::hash::list)
			{
				if (single_config.name == dog_params["type"].to_string())
				{
					hash_config = std::make_unique<dog_torch::crypto::hash::HashConfig>(single_config);
					break;
				}
			}
			if (!hash_config)
			...
			if (!dog_torch::math::region::is_fall(hash_config->region, dog_params["effective"].to_number()))
			{
				throw DOG_EXCEPTION("有效输出数不在范围内");
			}
			dog_torch::crypto::hash::HashCrypher hash_crypher = dog_torch::crypto::hash::HashCrypher(hash_config->name, dog_params["effective"].to_number());
			if (input__config.is_data())
			{
				dog_work::Timer timer;
				timer.start();
				result["res"] = QString::fromStdString(output_config.fmt_data(hash_crypher.get_data_hash(input__config.get_data())));
				timer.end();
				result["time"] = timer.get_time();
				result["msg"] = "散列成功";
			}
...
public slots:
	void test(const QString& jsonStr)
	{
		...
		dog_torch::crypto::hash::HashCrypher hash(type, effective);
...
public slots:
	void work(const QString& jsonstr)
	{
		...
		try
		{
			...
			dog_torch::serialize::json::Object dog_params = std_json_str;
			////std::cout << dog_params.to_string(true, 0) << std::endl;
			dog_param::IOConfig input__config = dog_param::IOConfig(dog_params["io"].to_object()["input"].to_object(), true);
			dog_param::IOConfig output_config = dog_param::IOConfig(dog_params["io"].to_object()["output"].to_object(), false);
			
			dog_torch::serialize::json::Object all_config = dog_params["config"].to_object();
			dog_torch::crypto::symmetric::CryptionConfig config = dog_torch::crypto::symmetric::CryptionConfig(
				all_config["type"].to_string(), all_config["blockSize"].to_number(), all_config["keySize"].to_number(),
				all_config["isPadding"].to_bool(), all_config["padding"].to_string(),
				all_config["mode"].to_string(), true, all_config["shift"].to_number()
			);

			dog_param::IOConfig key_config = dog_param::IOConfig(dog_params["key"].to_object()["input"].to_object(), true);
			std::unique_ptr<dog_param::IOConfig> iv_config = nullptr;
			auto iv_params = dog_params["iv"].to_object();
			if (iv_params.find("auto") != iv_params.end())
			{
				std::unordered_map<std::string, std::any> iv_params;
				iv_params["ori_str"] = dog_torch::crypto::symmetric::utils::randiv(config.block_size).to_hex_string();
			...
			{
				iv_config = std::make_unique<dog_param::IOConfig>(iv_params["input"].to_object(), true);
			}

			dog_torch::serialize::json::Object head = dog_params["head"].to_object();
			bool with_check = head["withCheck"].to_bool();
			bool with_config = head["withConfig"].to_bool();
			bool with_iv = head["withIV"].to_bool();

			dog_torch::crypto::symmetric::Cryptor cryptor = dog_torch::crypto::symmetric::Cryptor(config);

			dog_torch::serialize::Data key_data = key_config.get_data();
			dog_torch::serialize::Data iv_data = iv_config->get_data();

			if (!input__config.is_file())
			{
				dog_torch::serialize::Data input_data = input__config.get_data();
				result["iv"] = QString::fromStdString(iv_data.to_hex_string());

				cryptor.set_key(key_data);
				dog_work::Timer timer;
				timer.start();
				dog_torch::serialize::Data output_data = cryptor.encrypt(input_data, with_config, with_iv, iv_data, with_check);
				timer.end();
				result["time"] = timer.get_time();
...
public slots:
	void test(const QString& jsonstr)
	{
		...
		std::unique_ptr<dog_torch::crypto::symmetric::AlgorithmConfig> algorithm_config = nullptr;
		...
		for (auto& item : dog_torch::crypto::symmetric::Algorithm_list)
		{
			if (item.name == params["type"].toString().toStdString())
			{
				algorithm_config = std::make_unique<dog_torch::crypto::symmetric::AlgorithmConfig>(item);
                break;
			}
		}
		if (!algorithm_config)
		...
		if (!dog_torch::math::region::is_fall(algorithm_config->key_size_region, key_size))
		{
			result["code"] = 1;
		...
		if (!dog_torch::math::region::is_fall(algorithm_config->block_size_region, block_size))
		{
			result["code"] = 1;
		...
		dog_torch::crypto::symmetric::Cryptor cryptor(algorithm_config->name, block_size, key_size, true, "PKCS7", "ECB", false, 0);
		dog_torch::serialize::Data block = dog_torch::crypto::symmetric::utils::randiv(block_size);
		dog_torch::serialize::Data key = dog_torch::crypto::symmetric::utils::randiv(key_size);
		cryptor.set_key(key);
		dog_work::Timer t;
		t.start();
	...
	void work(const QString& jsonstr)
	{
		...
		try
		{
			...
			if (!input__config.is_file())
			{
				...
				dog_work::Timer timer;
				timer.start();
				dog_torch::serialize::Data output_data = cryptor.decrypt(input_data, with_config, with_iv, iv_data, with_check);
				timer.end();
				result["time"] = timer.get_time();
				result["res"] = QString::fromStdString(output_config.fmt_data(output_data));
				result["msg"] = "解密成功";
...
public:
	...
	bool check()
	{
		...
		dog_torch::crypto::hash::HashCrypher crypher("SHA2", 32);
		uint64_t i = 0;
		for (auto& file : files_hash)
		{
			...
			std::string hash_str = dog_torch::crypto::hash::HashCrypher::streamHash(crypher, file_stream).to_hex_string();
			crypher.init();
