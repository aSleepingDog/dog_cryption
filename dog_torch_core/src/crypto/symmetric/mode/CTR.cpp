...
DOG_DATA NSROOT::mode::CTR::encrypt(const Data& plain, const Cipher& cipher)
{
	...
	Data tempBlock0 = ((const CTR&)cipher.get_mode()).get_iv();
	uint64_t endNum = 0;
	...
	padding::padding_func padding = ((const CTR&)cipher.get_mode()).get_padding().get_padding();
...
DOG_DATA NSROOT::mode::CTR::decrypt(const Data& crypt, const Cipher& cipher)
{
	uint64_t block_size = cipher.get_block_size();
	uint64_t key_size = cipher.get_key_size();
	Data tempBlock0 = ((const CTR&)cipher.get_mode()).get_iv();
	algorithm::block_self_cryption_func block_self_encryption = cipher.get_block_self_encryption();
	padding::padding_func unpadding = ((const CTR&)cipher.get_mode()).get_padding().get_unpadding();
	const Data& key = cipher.get_available_key();
...
void NSROOT::mode::CTR::encrypt_stream(std::istream& plain, std::ostream& crypt, const Cipher& cipher)
{
...
void NSROOT::mode::CTR::encrypt_streamp(std::istream& plain, std::ostream& crypt, const Cipher& cipher,
	std::mutex* mutex_, std::condition_variable* cond_, std::atomic<double>* progress_, std::atomic<bool>* running_, std::atomic<bool>* paused_, std::atomic<bool>* stop_)
{
	...
	tempBlock2 = tempBlock0;
	block_self_encryption(tempBlock0, block_size, key, key_size); plain.read((char*)tempBlock1.data(), block_size);
	for (uint64_t i = 0; i < block_size - plain.gcount(); ++i) { tempBlock1.pop_back(); }
	if ( plain.gcount() < block_size)
	{
		padding(tempBlock1, block_size);
...
void NSROOT::mode::CTR::decrypt_streamp(std::istream& crypt, std::ostream& plain, const Cipher& cipher,
	std::mutex* mutex_, std::condition_variable* cond_, std::atomic<double>* progress_, std::atomic<bool>* running_, std::atomic<bool>* paused_, std::atomic<bool>* stop_)
{
	uint64_t now_pos = crypt.tellg();
	crypt.seekg(now_pos, std::ios::end);
...
const DOG_DATA& dog_torch::crypto::symmetric::mode::CTR::get_iv() const
{
	return iv_;
}
const NSROOT::padding::Padding& dog_torch::crypto::symmetric::mode::CTR::get_padding() const
{
	return *padding_;
}
