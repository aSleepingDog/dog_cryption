...
int main()
{
	...
	dog_torch::crypto::hash::HashCrypher hash_crypher(dog_torch::crypto::hash::SHA2::name, dog_torch::crypto::hash::SHA2::b256::EFFECTIVE_SIZE);
	hash_crypher.init();
	for (auto fit : std::filesystem::directory_iterator(page_path))
	{
		if (fit.is_directory())
		{
			for (auto fit0 : std::filesystem::directory_iterator(fit))
			{
				...
				dog_torch::serialize::Data hash = dog_torch::crypto::hash::HashCrypher::streamHash(hash_crypher, input);
				file_and_hash.emplace_back(std::pair<std::string, std::string>('\"' + file_name + "\",", '\"' + hash.to_hex_string() + '\"'));
				hash_crypher.init();
			}
		}
		else
