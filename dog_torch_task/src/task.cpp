...
void dog_work::PausableThread::run(std::string type, Task* task, std::unordered_map<std::string, std::any>* params)
{
    if (type == "hash")
    {
        auto dog_work = [this, params, task]()->void
            {
                dog_torch::crypto::hash::HashCrypher hash_crypher = std::any_cast<dog_torch::crypto::hash::HashCrypher>((*params)["hash_crypher"]);
                ...
                task->timer.start();
                dog_torch::crypto::hash::HashCrypher::streamHashp(hash_crypher, input, std::any_cast<dog_torch::serialize::Data*>((*params)["result"]),
        ...
        auto dog_work = [this, params, task]()->void
            {
                try
                {
                    dog_torch::crypto::symmetric::Cryptor cryptor = std::any_cast<dog_torch::crypto::symmetric::Cryptor>((*params)["cryptor"]);
                    ...
                    dog_torch::serialize::Data iv = std::any_cast<dog_torch::serialize::Data>((*params)["iv"]);
                    task->timer.start();
        ...
        auto dog_work = [this, params, task]()->void
            {
                ...
                catch (dog_torch::crypto::symmetric::WrongKeyException& e)
                {
                    task->set_msg("密钥校验失败,请输入正确的密钥");
                }
                catch (dog_torch::crypto::symmetric::WrongConfigException& e)
                {
...
dog_work::Task::Task(uint64_t id, std::string input, dog_torch::crypto::hash::HashCrypher hash_crypher, std::unordered_map<std::string, std::any> output_params)
{
    this->id_ = id;
...
dog_work::Task::Task(uint64_t id, int type, std::string input, std::string output, dog_torch::crypto::symmetric::Cryptor& cryptor, 
    dog_torch::serialize::Data iv, bool with_config, bool with_iv, bool with_check)
{
    this->id_ = id;
    this->params_ = new std::unordered_map<std::string, std::any>();
    if (type == 1)
...
std::unordered_map<std::string, std::any> dog_work::Task::get_info()
{
    ...
    if (this->type_ == "hash")
    {
        res["hash"] = std::any_cast<dog_torch::crypto::hash::HashCrypher>(this->params_->at("hash_crypher")).get_config();
        ...
        if (std::any_cast<int>(res["status"]) == 2)
        {
            ...
            case 0:
            {
                res["result"] = this->result_.to_utf8_string();
                break;
            }
            case 1:
            {
                ...
                res["result"] = this->result_.to_base64_string(replace0, replace1, replace2);
                break;
            }
            case 2:
            {
                bool upper = std::any_cast<bool>(this->params_->at("upper"));
                res["result"] = this->result_.to_hex_string(upper);
                break;
            }
            }
            res["msg"] = "完成";
    ...
    {
        res["config"] = std::any_cast<dog_torch::crypto::symmetric::Cryptor>(this->params_->at("cryptor")).get_config().to_string()
...
uint64_t dog_work::TaskPool::add_hash(std::string path, dog_torch::crypto::hash::HashCrypher& hash_crypher, std::unordered_map<std::string, std::any> output_params)
{
...
uint64_t dog_work::TaskPool::add_hash(dog_param::IOConfig input_config, dog_torch::crypto::hash::HashCrypher& hash_crypher, dog_param::IOConfig output_config)
{
    uint64_t id = this->id_.load();
    this->id_.fetch_add(1);
...
uint64_t dog_work::TaskPool::add_encrypt(std::string input_path, std::string output_path, dog_torch::crypto::symmetric::Cryptor& cryptor, 
    dog_torch::serialize::Data iv, bool with_config, bool with_iv, bool with_check)
{
    uint64_t id = this->id_.load();
    this->id_.fetch_add(1);
...
uint64_t dog_work::TaskPool::add_decrypt(std::string input_path, std::string output_path, dog_torch::crypto::symmetric::Cryptor& cryptor, 
    dog_torch::serialize::Data iv, bool with_config, bool with_iv, bool with_check)
{
    uint64_t id = this->id_.load();
    this->id_.fetch_add(1);
