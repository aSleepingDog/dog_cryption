...
#define DOG_ERROR_BAD_BLOCK "Error:The data is not encrypted by CBC mode"
#define DOG_ERROR_NONE_PADDING "Error:CBC mode not support No padding"

NSROOT::mode::Config NSROOT::mode::CBC::get_config()
{
    return 
    {
    "CBC",
...
DOG_DATA NSROOT::mode::CBC::decrypt(const Data& crypt, const Data& available_key, const algorithm::Algorithm& algorithm, const Data& iv, padding::padding_func unpadding)
{
    ...
    for (; block_size < crypt.size() - i; i += block_size)
    {
        ...
        dog_torch::serialize::BinaryData::XOR_self(temp_block1, temp_block0, block_size);
        plain += temp_block1;
        temp_block0 = crypt.sub_bytes_by_len(i, block_size);
    }
    temp_block1 = crypt.sub_bytes_by_len(i, block_size);
    block_self_decryption(temp_block1, block_size, key, key_size);
    dog_torch::serialize::BinaryData::XOR_self(temp_block1, temp_block0, block_size);
...
void NSROOT::mode::CBC::encryptp_stream(PauseableChannel& pchannel, std::istream& plain, uint64_t max, std::ostream& crypt, const Data& available_key, const algorithm::Algorithm& algorithm, const Data& iv, padding::padding_func padding)
{
    CBC_ENCRYPT_INIT;
...
NSROOT::mode::streamp_crypt_func NSROOT::mode::CBC::get_streamp_encrypt() const
{
...
NSROOT::mode::streamp_crypt_func NSROOT::mode::CBC::get_streamp_decrypt() const
{
