...
dog_torch::crypto::hash::HashCrypher get_hash_type()
{
    std::string input = "";
    std::vector<std::string> params;
    std::cout << "请输入散列类型[hash] [type] 可选项如下 示例SHA2 256" << std::endl;
    for (auto one : dog_torch::crypto::hash::list)
    {
        std::cout << std::format("{} |", one.name);
        std::cout.flush();
        dog_torch::math::region::NumberIterator nit(one.region);
        while (nit.have_next())
        {
    ...
    for (auto one : dog_torch::crypto::hash::list)
    {
        if (one.name == args[0])
        {
            if (dog_torch::math::region::is_fall(one.region, number/8))
            {
                return dog_torch::crypto::hash::HashCrypher(one.name, number / 8);
            }
            else
            {
...
dog_param::IOConfig get_iv(uint64_t block_size)
{
    ...
    if (args[0] == "y")
    {
        ...
        iv_params["ori_str"] = dog_torch::crypto::symmetric::utils::randiv(block_size).to_hex_string();
...
{
    dog_torch::crypto::symmetric::CryptionConfig config;
    bool with_config;
    bool with_iv;
    bool with_check;
};
CryptionParams get_cryption_type()
{
    ...
    for (auto one : dog_torch::crypto::symmetric::Algorithm_list)
    {
        std::cout << std::format("{} |", one.name);
        dog_torch::math::region::NumberIterator nit(one.block_size_region);
        while (nit.have_next())
        {
            std::cout << nit.next() * 8 << " ";
        }
        std::cout << "|";
        nit = dog_torch::math::region::NumberIterator(one.key_size_region);
        while (nit.have_next())
        {
            std::cout << nit.next() * 8 << " ";
        }
    ...
    for (auto one : dog_torch::crypto::symmetric::Algorithm_list)
    {
        if (one.name == args[0])
        {
            if (dog_torch::math::region::is_fall(one.block_size_region, block_size) && dog_torch::math::region::is_fall(one.key_size_region, key_size))
            {
    ...
    for (auto one : dog_torch::crypto::symmetric::mode::list)
    {
        std::cout << std::format("{} | {} | {} | {} \n", 
            one.name, 
            one.force_iv ? "强制使用iv" : "不强制使用iv", 
            one.force_padding ? "强制使用填充" : "不强制使用填充", 
            one.force_shift ? "需要偏移" : "不需要偏移");
    }
    ...
    for (auto one : dog_torch::crypto::symmetric::padding::list)
    {
        std::cout << std::format("{}->{}|", one.name, one.code);
    }
    std::cout << std::endl;
    auto args3 = get_args();
    if (args3.size() != 1)
    ...
    for (auto one : dog_torch::crypto::symmetric::padding::list)
    {
        if (one.code == padding_code)
        {
            padding_name = one.name;
        }
    }
    ...
    dog_torch::crypto::symmetric::CryptionConfig config_ = {
        ...
        try
        {
            ...
            case 2:
            {
                ...
                if (!input_config.is_file())
                {
                    ...
                    dog_work::Timer timer;
                    timer.start();
                    std::string result = output_config.fmt_data(hash_crypter.get_data_hash(input_config.get_data()));
                    timer.end();
                    std::cout << "处理文本" << std::endl;
                    std::cout << input_config.get_ori_str() << std::endl;
                    std::cout << "转换方式" << std::endl;
            ...
            case 3:
            {
                ...
                if (!input_config.is_file())
                {
                    ...
                    clear_print();
                    dog_torch::crypto::symmetric::Cryptor cryptor(cryption.config);
